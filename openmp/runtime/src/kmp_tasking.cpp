...
#if OMPX_TASKGRAPH
  if (!tasknew->is_taskgraph &&
      UNLIKELY(node && (node->dn.mtx_num_locks > 0))) {
#else
...
// Make sure task & taskdata are aligned appropriately
  ...
  if (sizeof_shareds > 0) {
    ...
    KMP_DEBUG_ASSERT((((kmp_uintptr_t)task->shareds) & (sizeof(void *) - 1)) ==
                     0);
    KA_TRACE(1, ("KMP_TASK_SHARED: offset=%d, size_of=%d, shared pointer=>%p\n",
                 shareds_offset, sizeof_shareds, task->shareds));
  } else {
    task->shareds = NULL;
  }
...
kmp_task_t *__kmpc_omp_task_alloc(ident_t *loc_ref, kmp_int32 gtid,
  ...
  KA_TRACE(1, ("__kmpc_omp_task_alloc(enter): T#%d loc=%p, flags=(%s %s %s) "
               "sizeof_task=%ld sizeof_shared=%ld entry=%p\n",
...
void __kmp_taskloop_linear(ident_t *loc, int gtid, kmp_task_t *task,
  ...
  KA_TRACE(2, ("%s:%d: __kmp_taskloop_linear: Elapsed scheduling overhead"
...
static void __kmp_taskloop(ident_t *loc, int gtid, kmp_task_t *task, int if_val,
  ...
  KA_TRACE(2, ("__kmp_taskloop: T#%d, task %p, lb %lld, ub %lld, st %lld, "
               "grain %llu(%d, %d), dup %p\n",
               gtid, taskdata, lower, upper, st, grainsize, sched, modifier,
               task_dup));
