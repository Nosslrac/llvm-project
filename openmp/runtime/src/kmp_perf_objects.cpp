...
AMDRawResults AMDRawResults::avg(int32_t nthreads) {
  return AMDRawResults(totDisp / nthreads, backend / nthreads,
                       backendMem / nthreads, backendCPU / nthreads);
}
...
AMDRawResults RawAMDPerfContainer::stopAndReadAll() {
  ...
  // TODO: Add the rest
  return AMDRawResults(totDisp, backendBoundFrac, backendBoundMemFrac,
                       backendBoundCPU);
}
