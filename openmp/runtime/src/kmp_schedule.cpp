...
namespace {
  ...
  void __kmp_alloc_task_deque(kmp_thread_data_t *thread_data, int32_t gtid)
  {
    ...
    KA_TRACE(2, ("%s:%d: __kmp_alloc_task_deque: Allocate task space for T#%d.\n ", 
  ...
  kmp_info_t *__kmp_select_thread(kmp_info *master_thread, kmp_team *team, int32_t nthreads)
  {
    // Round robin scheduling
    master_thread->th.next_thread = (master_thread->th.next_thread + 1) % nthreads;
...
kmp_thread_data_t* Schedule::__kmp_optimal_thread(kmp_info *master_thread, kmp_task_team *task_team)
{
  int32_t nthreads = task_team->tt.tt_nproc;
  kmp_team *team = master_thread->th.th_team;
  kmp_info_t *rand_thread = __kmp_select_thread(master_thread, team, nthreads); // Get random thread
  ...
  KA_TRACE(1, ("%s:%d: __kmp_get_random_deque: Finding random deque to add task T#%d.\n ", 
